...
const aggregateData = (data, selectedsector) => {
  return data.reduce((acc, item) => {
    if (item.sector === selectedsector && item.region !== "") {
      if (acc[item.region]) {
        acc[item.region] += 1;
      } else {
        acc[item.region] = 1;
...
const PieChartOfsectorWiseregion = ({ data, selectedsector }) => {
  const d3Container = useRef(null);

  useEffect(() => {
    if (data.length > 0) {
      const aggregatedData = aggregateData(data, selectedsector);
...
export default PieChartOfsectorWiseregion;

...
const aggregateData = (data) => {
  ...
  data.forEach(item => {
    const year = item.start_year === "" ? "Unknown" : item.start_year;
    if (!result[year]) {
      result[year] = {};
    }
...
const HorizontalBarChartYearTopic = ({ data }) => {
    ...
    if (data.length > 0) {
      ...
      svg.append('g')
        ...
        .on('mouseover', function(event, d) {
          ...
          tooltip.html(`Year: ${d.data[0].year}<br>Topic: ${d.data.find(s => s.count === d[1] - d[0]).topic}<br>Count: ${d[1] - d[0]}`)
  ...
  return (
    <div>
    <h2 style={{ textAlign: 'center', marginTop: '20px', marginBottom: '20px' }}>Topic By Start year</h2>
      <svg
        ref={d3Container}
        width={1000}
        height={600}
...
export default HorizontalBarChartYearTopic;

...
const aggregateData = (data) => {
  ...
  return Object.keys(result).map(sector => ({
    sector: sector,
    relevance: result[sector].total / result[sector].count
  }));
};

const HorizontalBarChartRelevanceBySector = ({ data }) => {
  ...
  useEffect(() => {
    if (data.length > 0) {
      ...
      formattedData = formattedData.filter(d => d.sector !== "Unknown");
      ...
      svg.append('g')
        ...
        .on('mouseover', function(event, d) {
          ...
          tooltip.html(`Sector: ${d.sector}<br>Average Relevance: ${d.relevance.toFixed(2)}`)
  ...
  return (
    <>
    <h2 style={{ textAlign: 'center', marginTop: '20px', marginBottom: '20px' }}>Horizontal Bar Chart - Sector by Average Relevance</h2>
      <svg
        ref={d3Container}
        width={800}
        height={400}
...
export default HorizontalBarChartRelevanceBySector;

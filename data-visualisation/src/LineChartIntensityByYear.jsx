...
const aggregateData = (data) => {
  const result = data.reduce((acc, item) => {
    ...
    if (acc[year]) {
      ...
      acc[year] = { total: item.intensity, count: 1 };
    }
    return acc;
  }, {});
  ...
  return Object.keys(result)
    ...
    .map(year => ({
      year: year,
      intensity: result[year].total / result[year].count
    }));
};

const LineChartIntensityByYear = ({ data }) => {
      ...
      svg.selectAll('.dot')
        ...
        .on('mouseover', function(event, d) {
          d3.select(tooltipRef.current)
            .style('opacity', 0.9)
            .html(`Year: ${d.year}<br>Average Intensity: ${d.intensity.toFixed(2)}`)
  ...
  return (
    <>
    <h2 style={{ textAlign: 'center', marginTop: '20px', marginBottom: '20px' }}>Horizontal Bar Chart - Average Intensity by End Year</h2>
...
export default LineChartIntensityByYear;
